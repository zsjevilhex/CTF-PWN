 ...
 */
// zsj-20190317-main_arena结构Mutex
...
// 原文：https://blog.csdn.net/qq_29343201/article/details/59614863 
...
# endif
  ...
  p = mem2chunk (mem);
  /* Two optional cases in which clearing not necessary 两种情况下不需要对申请的内存清零*/
  //判断当前内存是否为mmapped分配，如果mmapped标志位为1，则看下p标志位，如果p标志位为0则对内存清零，否则直接返回不清零
  if (chunk_is_mmapped (p))
    {
